...
if ($err) {
  echo "cURL Error #:" . $err;
	...
	//pr($List); exit;
	$texifleetpromotions=$List->response_object;
}
			...
			<?php $i=0;		
				...
				<div class="row col-md-12"> 
					<div class="col-md-4" style="margin-top: 7px;">
						<span style="color:#727e8a">Name </span> : <?php $hrefurl =  $this->Url->build(array('controller'=>'users','action'=>'viewprofile',$texifleetpromotion->user_id),1);?>
						<a href="<?php echo $hrefurl; ?>"> 
						<?php echo $texifleetpromotion->user->first_name." ".$texifleetpromotion->user->last_name;?></a>
					</div>
					<div class="col-md-4" style="margin-top: 7px;">
				...
				if ($user_id!=$texifleetpromotion->user_id)
					{
						if (in_array($texifleetpromotion->user_id,$follower_list))
						{
						?>
				...
				btn btn-info btn-sm"  data-target="#follow<?php echo $texifleetpromotion->id; ?>" data-toggle=modal>follow</a>
					...
					<div id="unfollow<?php echo $texifleetpromotion->id; ?>" class="modal fade" role="dialog">
						<div class="modal-dialog modal-md" >
							 
							<form method="post" class="formSubmit">
							<input type="hidden" name="follow_id" value="<?php echo $texifleetpromotion->user_id; ?>">
					...
					<div id="follow<?php echo $texifleetpromotion->id; ?>" class="modal fade" role="dialog">
						<div class="modal-dialog modal-md" >
							<form method="post" class="formSubmit">
							<input type="hidden" name="follow_id" value="<?php echo $texifleetpromotion->user_id; ?>">
								<div class="modal-content">
				...
				if ($user_id!=$texifleetpromotion->user_id)
					{
						...
						{
						?>
					<a  class=" btn btn-danger btn-md"  data-target="#unfollow<?php echo $texifleetpromotion->id; ?>" data-toggle="modal" >Unfollow</a>
					<?php }
					else{
						?>
				<a follow_id="<?php echo $texifleetpromotion->user_id; ?>" class=" 
				btn btn-success btn-md"  data-target="#follow<?php echo $texifleetpromotion->id; ?>" data-toggle=modal>follow</a>
					<?php }}?>
