...
class PostTravlePackagesController extends AppController
{
    ...
    public function add()
    {
        ...
        if ($this->request->is('post')) {
			...
			if(!empty($image))
			{	
				$dir = new Folder(WWW_ROOT . 'images/PostTravelPackages/'.$id.'/image', true, 0755);
				...
				if(!empty($ext))
				{
					if(in_array($ext, $arr_ext)) { 
						...
						if(move_uploaded_file($image['tmp_name'], WWW_ROOT . '/images/PostTravelPackages/'.$id.'/image/'.$id.'.'.$ext)) {
							$postTravlePackage->image='images/PostTravelPackages/'.$id.'/image/'.$id.'.'.$ext;
						} else {
			...
			if(!empty($document))
			{  
				$dir = new Folder(WWW_ROOT . 'images/PostTravelPackages/'.$id.'/document', true, 0755);
				...
				if(!empty($ext))
				{
					if (in_array($ext, $arr_ext)) {
						...
						if(move_uploaded_file($document['tmp_name'], WWW_ROOT . '/images/PostTravelPackages/'.$id.'/document/'.$id.'.'.$ext)) {
							$postTravlePackage->document='images/PostTravelPackages/'.$id.'/document/'.$id.'.'.$ext;
						} else {
