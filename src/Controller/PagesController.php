...
->toArray();        
    ...
    public function myfinalresponsesapi() {
		...
		foreach($responses as $cit)
		{
			$query = $this->Testimonial->find();
			$userRating = $query->select(["average_rating" => $query->func()->avg("rating")])
					->where(['author_id' => $cit['request']['user_id']])
					->order(["id" => "DESC"]);
...
$query = $this->Testimonial->find();
$userRating = $query->select(["average_rating" => $query->func()->avg("rating")])
						->where(['author_id' => $row['request']['user_id']])
						->order(["id" => "DESC"]);
					$rating['rating'][$row['id']]  = $userRating;
	...
	exit;
		...
		public function finalizedrequestlistapi() {
			...
			foreach($requests as $cit)
			{
				...
				$query = $this->Testimonial->find();
				$userRating = $query->select(["average_rating" => $query->func()->avg("rating")])
					->where(['author_id' => $cit['response']['user_id']])
					->order(["id" => "DESC"]);
				$rating['rating'][$cit['response']['user_id']]  = $userRating;
...
//-- Rating Update 
	...
	public function acceptofferapi() {
		...
		if ($TableRequest->save($request)) {
			...
			$message = "$name has accepted your offer. Please go to Finalized Requests/Reponses to add a Review for $SendToUserName";
			$msg = "$name has accepted your offer. Please go to Finalized Requests/Reponses to add a Review for $SendToUserName.";
